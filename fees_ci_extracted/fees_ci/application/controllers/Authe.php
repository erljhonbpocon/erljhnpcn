...
class Authe extends CI_Controller {
	...
	function login()
	{
		...
		if ($settings && isset($settings->MAINTENANCE_MODE) && $settings->MAINTENANCE_MODE == 1) {
			// Show maintenance page instead of login
			$this->load->view('maintenance_login');
		} else {
	...
	function valafclog()
	{
		...
		if ($check == 'active_staff') {
			...
			// Check if maintenance mode is ON for parent/client login
			$settings = $this->db->get_where('settings_tbl', array('ID' => 1))->row();
			if ($settings && isset($settings->MAINTENANCE_MODE) && $settings->MAINTENANCE_MODE == 1) {
				// Destroy the session that was just created
				$this->session->sess_destroy();
				$this->session->set_flashdata('maintenance_notice', 'The system is currently under maintenance. Please try again later.');
