...
class Admin extends CI_Controller {
	...
	function management($name='')
	{
		...
		if ($name=='maintenance') {
			...
			$page_data['page_title'] = 'Maintenance Mode';
			$page_data['page_s_name'] = 'maintenance';

			$this->load->view('admin/maintenance', $page_data);
		}
	}
	...
	function toggle_maintenance() {
		if ($this->session->userdata('login_check') != 'go@yes' || $this->session->userdata('login_priv') != 'admin') {
		...
		$this->db->where('ID', 1);
		$this->db->update('settings_tbl', $data);

		if ($this->db->affected_rows() > 0) {
			echo json_encode(array('status' => 'success', 'message' => 'Maintenance mode ' . ($maintenance_status == 1 ? 'enabled' : 'disabled')));
		} else {
			echo json_encode(array('status' => 'error', 'message' => 'Failed to update maintenance mode'));
		}
	}
	...
	function get_maintenance_status() {
		...
		$settings = $this->db->get_where('settings_tbl', array('ID' => 1))->row();
		$maintenance_mode = isset($settings->MAINTENANCE_MODE) ? $settings->MAINTENANCE_MODE : 0;
		
		echo json_encode(array('status' => 'success', 'maintenance_mode' => $maintenance_mode));
	}
}
